...
public class CSVParser {
    ...
    private final EntityManager em; //ParkService와 CSVParser의 순환참조를 막기 위해 em 사용
    ...
    public void read() {
        ...
        try {
            //utf-8 형태로 csv 파일 파싱
            CSVReader csvReader = new CSVReader(new InputStreamReader(new FileInputStream(path + "\\src\\main\\java\\daone\\bmap\\csv\\data.csv"), "EUC-KR"));
            ...
            do {    //파일에서 데이터를 읽어 파싱하고 Park 객체로 만들어 ArrayList 에 넣는다.
                ...
                if (parkInfo != null) {
                    if (parkInfo[28] == null || parkInfo[29] == null || parkInfo[28].isEmpty() || parkInfo[29].isEmpty())  //읽어온 데이터의 위도, 경도 값이 없거나 null 이면 저장하지 않고 넘김
                        ...
                        em.persist(ParkMapper.mapper.parkDtoToEntity(parkDto));   //park 객체를 DB에 INSERT
                    }
                }
            } while (parkInfo != null);
        } catch (IOException e) {
